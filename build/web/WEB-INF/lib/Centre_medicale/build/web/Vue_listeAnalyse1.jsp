<%-- 
    Document   : Vue_listeAnalyse1
    Created on : 16 févr. 2018, 11:54:55
    Author     : Delex
--%>
            ...
            function supprimer_mati(nbmax) {
                ...
                if (exist == 0)
                    ...
                    if (confirm('Si vous allez supprimer cet élement, vous risque de perdre son contenu!!\n \n \f NB: Effectuer pour\t [ OK ou ANNULER ]:\n \n \f -OK :: Pour éliminer cet élement! \n \n \f - Annuler :: Pour garder encore cet élement!'))
                        document.location = "Supprimer_listeAnalyse.jsp?nbsuppr=" + exist + ident;
            }
            ...
            function modifier_matie(nbmax) {
                ...
                if (exist > 1)
                    ...
                    if (confirm('Vous êtes entrain de mêtre à jour une liste qui vient d/être sélectionner!! \n \n \f NB: Apuiez sur \t [ OK ou ANNULER ]:\n \n \f -OK :: Pour effectuer le changement ! \n \n \f - Annuler :: Pour ne pas changer !'))
                        document.location = "form_modif_analyse.jsp?code=" + ident;
                }
            }
                            ...
                            <form id="form1" name="form1" method="post" action="">
                                <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                                    ...
                                    <tbody>
                                        <% while (res.next()) {%>
                                        <tr>
                                            <td><%=res.getString("anal_numFacture")%>
                                                <input type="hidden" name="id<%=i%>" value="<%=res.getString("anal_numFacture")%>"/>
                                            </td>
                                            <td><%=res.getString("anal_idLaboratoire")%></td>
                                            <td><%=res.getString("anal_nomPrenom")%></td>
                                            <td><%=res.getString("anal_rc")%> </td> 
                                            <td><%=res.getString("anal_date")%> </td> 
                                            <td><%=res.getString("anal_priseCharge")%> </td> 
                                            <td><%=res.getString("anal_numTelpatient")%> </td> 
                                            <td><%=res.getString("anal_agePatient")%></td>
                                            <td><%=res.getString("anal_moisCor")%></td>
                                            <td><%=res.getString("anal_sexe")%> </td> 
                                            <td><%=res.getString("res_parametres")%> </td> 
                                            <td><%=res.getString("res_unite")%> </td> 
